pipeline {
    agent any
    environment {
        AWS_REGION = 'ap-south-1'//ur AWS region
        ECR_REPO = '945219712485.dkr.ecr.ap-south-1.amazonaws.com/my_pvt_repo' // Your ECR repository URI
        IMAGE_TAG = "v${BUILD_NUMBER}" // Dynamic version based on build number
        EKS_CLUSTER_NAME = 'kg_cluster' // Replace with your EKS cluster name
        KUBECONFIG_PATH = '/opt/kube/config' // Path to kubeconfigfile
            ...
            steps {
                script {
                    withCredentials([[
                        ...
                        credentialsId: 'aws-credentials-gk' // Your Jenkins AWS credentials ID
                    ]]) {
        ...
        stage('Deploy on EKS') {
            steps {
                script {
                    withCredentials([[
                        $class: 'AmazonWebServicesCredentialsBinding',
                        credentialsId: 'aws-credentials-gk' // Replace with your Jenkins AWS credentials ID
                    ]]) {
